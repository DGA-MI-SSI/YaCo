...
namespace yadiff { // All file long
...
#define RELATION_IDX_NONE ((relation_idx_t)-1)
    ...
    void MergeRelationFlags(Relation& dest, const Relation& src)
    {
        ...
        // Is newer (src) relation better ?
        bool newer_relation_is_better = false;
        if(dest.type_ == RELATION_TYPE_NONE
            && src.type_ != RELATION_TYPE_NONE)
        {
            newer_relation_is_better = true;
        }
        else if(dest.type_ == RELATION_TYPE_WEAK_MATCH)
        {
            if(src.type_ == RELATION_TYPE_ALTERNATIVE_SOLVED
        ...
        {
            if(src.type_ == RELATION_TYPE_DIFF
                || src.type_ == RELATION_TYPE_STRONG_MATCH
                || src.type_ == RELATION_TYPE_EXACT_MATCH)
            {
                newer_relation_is_better = true;
        ...
        {
            if(src.type_ == RELATION_TYPE_STRONG_MATCH
                || src.type_ == RELATION_TYPE_EXACT_MATCH
        ...
        else if(dest.type_ == RELATION_TYPE_STRONG_MATCH
            && src.type_ == RELATION_TYPE_EXACT_MATCH)
        {
            newer_relation_is_better = true;
        }

        // If newer (src) relation worth more than older (dest)
    ...
    // Append a relation -> YaDiffRelationContainer
    ...
    // Insert relation -> YaDiffRelationContainer (TODO read and document)
...
//        RelationType_e existing_relation_type;
        ...
        // TODO : Matching theory is in your heart, have faith !
        /*
        if(relation.type_ == RELATION_TYPE_ALTERNATIVE_TO_N)
        {
//            LOG(INFO, "Adding relation ALT_TO_N with 0x%016zu\n", relation.version1_.address());
        ...
        {
//            LOG(INFO, "Adding relation ALT_FR_N with 0x%016zu\n", relation.version2_.address());
...
//                LOG(INFO, "existing_relation_updated : %ld\n", relations_updated.size());
    ...
    // Reset new_relation_counter member
    ...
    // Walk relation calling callback
...
bool Matching::Analyse(std::vector<Relation>& output)
{
    ...
    YaDiffRelationContainer relations = YaDiffRelationContainer();
    ...
    // 1. Apply external mapping match algo
    if(config_.IsOptionTrue(SECTION_NAME, "ExternalMappingMatch"))
    {
        ...
        // Make DiffAlgo : ExternalMappingMatch <- external file mapping
        auto algo = MakeDiffAlgo(AlgoConfig);
        ...
        // Dye if cannot prepare
        else {
            LOG(ERROR, "could not apply external mapping\n");
        }
    }

    // 2. Execute ExactMath algo
    AlgoCfg AlgoConfig;
    ...
    // 3. Execute Algos_ in loop until nothing is found
