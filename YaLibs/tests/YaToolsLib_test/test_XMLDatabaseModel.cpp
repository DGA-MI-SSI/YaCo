...
namespace
{
    std::shared_ptr<std::queue<std::string>> read_xml_call_queue(const fs::path& filename, const char* input)
    {
        std::ofstream ofs;
...
{
    const fs::path filename("0123456789ABCDEF.xml");
    auto call_queue = read_xml_call_queue(filename, "<?xml version=\"1.0\" encoding=\"iso-8859-15\"?>\n\
<sigfile>\n\
  <basic_block>\n\
...
{
    const fs::path filename("0123456789ABCDEF.xml");
    auto call_queue = read_xml_call_queue(filename,
